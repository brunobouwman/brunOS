...
**Brain-local vs shared skills.** `brunos-vault` + `memory-search` are **vault-unique** — they describe THIS brain's folders / retrieval paths, so they're **gitignored** (`.gitignore` → `.claude/skills/brunos-vault/`, `.claude/skills/memory-search/`) and NOT propagated by code-sync (a shared copy would mis-teach another brain). They still live under `.claude/skills/` so `setting_sources=["project"]` loads them; each brain's copy is **created during bootstrap** ("seed vault skills" step, `onboarding_installer.md`) — BrunOS's are hand-created and kept locally per host (Mac + VPS each need their own copy, since code-sync won't carry them). The other skills (`news-digest`, `weekly-review`, `diagnose-brain`, `company-*`, `code-review`, `dev-task`, …) are shared code.

- `brunos-vault` (brain-local) — pure-context skill teaching folder layout, frontmatter, language routing, draft lifecycle, and boundaries (Slack carve-out, no `personal/finance.md`). No scripts. Triggers on most vault interactions. The universal target name is `vault-structure` (bootstrap generates it per brain); BrunOS's instance keeps the legacy name `brunos-vault`.
- `memory-search` (brain-local) — pure-context skill teaching when to invoke `memory_search.py`, query phrasing for asymmetric BGE, the `--path-prefix` folder cheat sheet, RRF score interpretation, the read-after-search workflow, and the fallback ladder. Triggers on most recall/search/dedup/tone-matching prompts. Pairs with `brunos-vault` (which teaches *where* things live; this one teaches *how to retrieve them by meaning*).
...
5. **Daemon path-boundary guard** — `.claude/hooks/path-boundary.py` (PreToolUse on `Bash|Edit|Write|MultiEdit|NotebookEdit`) makes two SOUL rules **hard** for autonomous, no-human-at-execution daemons (`CLAUDE_INVOKED_BY ∈ {chat, heartbeat}` — `GUARDED_CONTEXTS`; interactive sessions + dev-task project worktrees pass through). It denies `Write`/`Edit` whose **resolved** target (symlinks + `..` collapsed) is outside the allowlist `{code-repo root, vault}`, and denies single-file destructive Bash (`rm`/`rmdir`/`unlink`/`shred`/`mv`/`truncate`/`find -delete`) **anywhere** (SOUL: never delete — archive or mark status). Bash reads (systemctl/journalctl/git status/`query.py`) are unaffected. Key fact: PreToolUse hooks fire **before** the permission check and hard-deny in ANY `permission_mode` (`dontAsk`/`acceptEdits`/`bypassPermissions`) — so for a daemon with no human to approve, the hooks ARE the enforcement, orthogonal to the approval prompt. Tests: `tests/test_path_boundary.py`. (Diagnosed by `diagnose-brain` J8(4).)

Hook order in `.claude/settings.json`: `block-secrets.py` first, `dangerous-bash.py` second, `protect-soul.py` third, `path-boundary.py` last. Hook input is JSON on stdin. `block-secrets.py`, `protect-soul.py`, and `path-boundary.py` soft-block with `{"decision":"block","reason":"..."}` on stdout; `dangerous-bash.py` hard-blocks with exit 2 and stderr. The bot/heartbeat load all four purely via `setting_sources=["project"]` (no programmatic hook registration) — so `setting_sources=["project"]` + these `settings.json` entries together ARE the daemon's enforcement.
