...
- [ ] Phase C.5 — LinOS node DEPLOY + end-to-end verification: node provisioned,
  ...
  Remaining: decide/enable LinOS reflect cadence, run any remaining slug
  migration review, enable Mac inbox-retire after dry-run review, build the
  VPS-side F2 retirement job (`BrunOS-processed AND LinOS-acked ELSE 15-day
  fallback`), and productize broad multi-channel company chat + company
