...
- **Phase 6/7 boundary**: chat bot now owns BOTH DMs AND channel @mentions (Socket Mode push). Phase 6 heartbeat must NOT also reply to mentions — `slack.mentions_since_last_run()` should be repurposed for read-only context (e.g. "you were mentioned 3 times since last tick") rather than auto-reply. Otherwise both processes will double-respond.
