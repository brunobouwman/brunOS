...
"""
    ...
    # draft PR via gh (must be authed on the host)
    pr_body = (
    ...
    pr = subprocess.run(
        ["gh", "pr", "create", "--draft", "--title", f"[dev-task] {title}",
         "--body", pr_body, "--head", m["branch"]],
        capture_output=True, text=True, cwd=str(wt),
    )
    pr_url = pr.stdout.strip().splitlines()[-1] if pr.returncode == 0 and pr.stdout.strip() else None
    if not pr_url:
        # PR creation failed (e.g. private-repo draft 422). Push succeeded, so the
        # branch is safe — record + report, don't pretend success.
        raise RuntimeError(f"gh pr create failed (branch pushed OK): {pr.stderr or pr.stdout}")
