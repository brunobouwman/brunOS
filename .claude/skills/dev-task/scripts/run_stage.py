...
async def _run_agent(prompt: str, *, cwd: Path, max_turns: int, model: str) -> str:
    ...
    # The FULL reasoning stream goes to the trace log via `_log` (below).
    # We RETURN only the agent's closing message — the last assistant text block,
    # or the authoritative ResultMessage.result — so callers (e.g. the PR body)
    # get the summary the prompt asks for, never the think-aloud narration.
    final_text = ""
            ...
            try:
                for block in content:
                    ...
                    if isinstance(t, str) and t.strip():
                        final_text = t  # keep only the latest text block
                        _log(f"· {t.strip()[:200]}")
                ...
                pass
        # ResultMessage carries the authoritative final result text — prefer it.
...
"""
    ...
    _log(f"PLAN agent finished ({len(out)} chars of closing summary)")
...
"""
    ...
    pr_body = (
        ...
        f"**Agent summary:**\n\n{(summary[:4000] or '_(no closing summary emitted — see trace log)_')}\n\n"
